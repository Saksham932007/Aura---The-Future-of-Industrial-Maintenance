...
class AuraDashboard {
    ...
    async init() {
        ...
        try {
            ...
            this.showError('Failed to load dashboard. Please check if the server is running.');
        }
    }
    ...
    updateSystemOverview(data) {
        ...
        if (systemHealthEl && data.system_health !== undefined) {
            ...
            systemHealthEl.className = `text-3xl font-bold ${this.getHealthColor(data.system_health)}`;
        }
        ...
        if (efficiencyEl && data.system_health !== undefined) {
            const efficiency = Math.min(100, Math.max(0, data.system_health + Math.random() * 10 - 5));
    ...
    createMachineCard(machine) {
        ...
        const alertBadge = machine.alert_level !== 'healthy' ? 
            ...
            <div class="relative">
                ...
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center space-x-3">
                        <div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center">
                            <i data-lucide="${this.getMachineIcon(machine.type)}" class="w-5 h-5 text-gray-600"></i>
                        </div>
                        <div>
                            <h3 class="font-semibold text-gray-900">${machine.name}</h3>
                            <p class="text-sm text-gray-500">${machine.type} • ${machine.location}</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-2">
                        <div class="status-indicator status-${machine.alert_level}"></div>
                        <span class="text-sm font-medium ${this.getAlertLevelColor(machine.alert_level)}">${machine.alert_level.toUpperCase()}</span>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-lg font-bold ${healthColor}">${machine.health_score}%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="h-2 rounded-full transition-all duration-500 ${this.getHealthBarColor(machine.health_score)}" 
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                    <div class="bg-gray-50 rounded-lg p-3">
                        ...
                        <p class="text-sm font-semibold text-gray-900">${machine.current_readings.temperature}°C</p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-3">
                        <div class="flex items-center space-x-2">
                        ...
                        <p class="text-sm font-semibold text-gray-900">${machine.current_readings.vibration}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                    <div class="bg-gray-50 rounded-lg p-3">
                        ...
                        <p class="text-sm font-semibold text-gray-900">${machine.current_readings.rotation_speed}</p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-3">
                        <div class="flex items-center space-x-2">
                        ...
                        <p class="text-sm font-semibold text-gray-900">${machine.current_readings.load}%</p>
                    </div>
                </div>
                ...
                ${machine.potential_issues.length > 0 ? `
                    <div class="bg-red-50 border border-red-200 rounded-lg p-3">
                        ...
                        <ul class="text-xs text-red-600 space-y-1">
                            ${machine.potential_issues.slice(0, 2).map(issue => `<li>• ${issue}</li>`).join('')}
                            ${machine.potential_issues.length > 2 ? `<li>• +${machine.potential_issues.length - 2} more...</li>` : ''}
                        </ul>
                    </div>
                ` : `
                ...
                <div class="mt-4 pt-3 border-t border-gray-100">
                    <div class="flex items-center justify-between text-xs text-gray-500">
                        <span>Last updated: ${this.formatTime(machine.last_updated)}</span>
    ...
    updateAlertsPanel() {
        ...
        alertsList.innerHTML = this.alerts.slice(0, 10).map(alert => `
            <div class="flex items-center space-x-4 p-3 bg-gray-50 rounded-lg border-l-4 ${this.getAlertBorderColor(alert.severity)}">
                <div class="flex-shrink-0">
                    <i data-lucide="${this.getAlertIcon(alert.severity)}" class="w-5 h-5 ${this.getAlertIconColor(alert.severity)}"></i>
                </div>
                <div class="flex-grow">
                    <p class="text-sm font-medium text-gray-900">${alert.message}</p>
                    <p class="text-xs text-gray-500">${this.formatTime(alert.timestamp)} • ${alert.machine_id}</p>
                </div>
                <div class="flex-shrink-0">
                    <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full ${this.getAlertBadgeColor(alert.severity)}">
    ...
    updateModalContent(data) {
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-2xl font-bold ${this.getHealthColor(machine.health_score)}">${machine.health_score}%</p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-gray-600">Status</label>
                        <p class="text-lg font-semibold ${this.getAlertLevelColor(machine.alert_level)}">${machine.alert_level.toUpperCase()}</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3 mt-2">
                        <div class="bg-white rounded-lg p-3 border">
                            ...
                            <p class="text-lg font-semibold">${machine.current_readings.temperature}°C</p>
                        </div>
                        <div class="bg-white rounded-lg p-3 border">
                            <div class="flex items-center space-x-2">
                                <i data-lucide="activity" class="w-4 h-4 text-blue-500"></i>
                            ...
                            <p class="text-lg font-semibold">${machine.current_readings.vibration}</p>
                        </div>
                        <div class="bg-white rounded-lg p-3 border">
                            <div class="flex items-center space-x-2">
                                <i data-lucide="rotate-cw" class="w-4 h-4 text-green-500"></i>
                            ...
                            <p class="text-lg font-semibold">${machine.current_readings.rotation_speed}</p>
                        </div>
                        <div class="bg-white rounded-lg p-3 border">
                            <div class="flex items-center space-x-2">
                                <i data-lucide="gauge" class="w-4 h-4 text-purple-500"></i>
                            ...
                            <p class="text-lg font-semibold">${machine.current_readings.load}%</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-900 mt-1 p-3 bg-white rounded-lg border">${machine.recommendation}</p>
                </div>
            </div>
        `;
        ...
        if (data.recent_alerts && data.recent_alerts.length > 0) {
            alertsContainer.innerHTML = data.recent_alerts.map(alert => `
                <div class="flex items-center space-x-3 p-3 bg-white rounded-lg border">
                    <i data-lucide="${this.getAlertIcon(alert.severity)}" class="w-4 h-4 ${this.getAlertIconColor(alert.severity)}"></i>
                    <div class="flex-grow">
                        <p class="text-sm font-medium text-gray-900">${alert.message}</p>
                        <p class="text-xs text-gray-500">${this.formatTime(alert.timestamp)}</p>
                    </div>
                </div>
            `).join('');
        } else {
        ...
        const maintenanceContainer = document.getElementById('modalMaintenanceHistory');
        if (maintenanceContainer) {
            if (data.maintenance_history && data.maintenance_history.length > 0) {
                maintenanceContainer.innerHTML = data.maintenance_history.slice(0, 5).map(log => `
                    <div class="flex items-center space-x-3 p-3 bg-white rounded-lg border">
                        <div class="flex-shrink-0">
                            <i data-lucide="${this.getMaintenanceIcon(log.activity_type)}" class="w-4 h-4 text-blue-500"></i>
                        </div>
                        <div class="flex-grow">
                            <p class="text-sm font-medium text-gray-900">${log.activity_type.charAt(0).toUpperCase() + log.activity_type.slice(1)}</p>
                            <p class="text-xs text-gray-600">${log.description}</p>
                            <p class="text-xs text-gray-500">${this.formatTime(log.timestamp)} • ${log.technician}</p>
                        </div>
                    </div>
                `).join('');
            } else {
    ...
    createModalCharts(data) {
        ...
        if (vibCtx && data.historical_readings) {
            ...
            this.charts.vibration = new Chart(vibCtx, {
                type: 'line',
                data: {
                    labels: vibData.map(d => new Date(d.timestamp).toLocaleTimeString()),
                    datasets: [{
                        label: 'Vibration',
            ...
            if (!this.isLoading) {
                try {
                    ...
                    if (this.selectedMachine) {
                        const response = await fetch(`${this.apiBaseUrl}/machine/${this.selectedMachine}`);
                        const data = await response.json();
                        this.updateModalContent(data);
                    }
                } catch (error) {
...
async function scheduleMaintenance() {
    if (window.dashboard && window.dashboard.selectedMachine) {
        ...
        document.getElementById('maintenanceForm').onsubmit = async (e) => {
            ...
            try {
                const response = await fetch(`${window.dashboard.apiBaseUrl}/maintenance`, {
                    method: 'POST',
                    headers: {
